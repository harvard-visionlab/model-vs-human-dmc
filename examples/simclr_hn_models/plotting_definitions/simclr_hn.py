...
__all__ = ['plotting_definition_simclr_hn']

def plotting_definition_simclr_hn(df):
    ...
    purple_shades = [rgb(128, 0, 128), rgb(148, 0, 211), rgb(186, 85, 211), rgb(221, 160, 221)]
    ...
    for i, model in enumerate([
        ...
        decision_makers.append(DecisionMaker(name_pattern=model,
                               color=green_shades[i], marker="^", df=df,
                               plotting_name=f"AlexNetW3 SimclrHN {i+1}"))        
    
    for i, model in enumerate([
        "resnet50_mlp_simclrnh_cdac593124_probe0",
        "resnet50_mlp_simclrnh_cdac593124_probe1",
        "resnet50_mlp_simclrnh_cdac593124_probe2",
        "resnet50_mlp_simclrnh_cdac593124_probe3",
    ]):
        decision_makers.append(DecisionMaker(name_pattern=model,
                               color=purple_shades[i], marker="<", df=df,
                               plotting_name=f"Resnet50_MLP SimclrHN {i+1}")) 
    ...
    #     decision_makers.append(DecisionMaker(name_pattern=model,
    #                            color=purple_shades[i], marker=">", df=df,
    #                            plotting_name=f"AlexNetLGN6W1 SimclrHN {i+1}")) 
    ...
    decision_makers.append(DecisionMaker(name_pattern="alexnet_anime_sgd_lr005",
                           color=rgb(255, 215, 0), marker="o", df=df,
                           plotting_name="AlexNetAnimeEdge"))
    
    decision_makers.append(DecisionMaker(name_pattern="resnet50",
                           color=red_shades[0], marker="v", df=df,
                           plotting_name="Resnet50 PyTorch"))
    
    decision_makers.append(DecisionMaker(name_pattern="clip",
                           color=red_shades[1], marker=">", df=df,
                           plotting_name="Clip_ViT-B/32"))
    
    decision_makers.append(DecisionMaker(name_pattern="clipRN50",
                           color=red_shades[2], marker=">", df=df,
                           plotting_name="Clip_ResNet50"))
...
def get_comparison_decision_makers(df, include_humans=True, humans_last=True):
    ...
    for i, model in enumerate([
        ...
        d.append(DecisionMaker(name_pattern=model,
                               color=purple_shades[i], marker="o", df=df,
                               plotting_name=f"Resnet50_MLP SimclrHN {i+1}")) 
    ...
    decision_makers.append(DecisionMaker(name_pattern="clip",
                           color=red_shades[1], marker="v", df=df,
                           plotting_name="Clip_ViT-B/32"))
    
    decision_makers.append(DecisionMaker(name_pattern="clipRN50",
                           color=red_shades[2], marker="v", df=df,
                           plotting_name="Clip_ResNet50"))
