...
def load_first_page_from_pdf(filepath, dpi=300):
    """
    Convert a PDF file to a PIL Image.
    This function returns the first page as a PIL Image.
    """
    # Convert the PDF; convert_from_path returns a list of PIL Image objects,
    # one per page. We assume your PDF has a single page.
...
def convert_shape_bias_to_texture_bias(data):
    # First, sort the categories alphabetically.
    sorted_categories_unsorted = data['sorted_categories']
    sorted_categories = sorted(sorted_categories_unsorted)
    
    # Build a dictionary for each decision maker:
    # We assume that the original 'class_avgs' order corresponds to sorted_categories_unsorted.
    # So we first map each category to its value, and then reassemble in sorted order.
    dm_summary = data['decision_maker_summary']
    data_dict = {}
    for dm, stats in dm_summary.items():
        # Create a mapping from category (as in the unsorted list) to its value.
        mapping = dict(zip(sorted_categories_unsorted, stats['class_avgs']))
        # Now create a list of values ordered by the sorted categories.
        ordered_values = [mapping[cat] for cat in sorted_categories]
        data_dict[dm] = pd.Series(ordered_values, index=sorted_categories)
    
    # Create a DataFrame with the decision makers as columns.
    df_results = pd.DataFrame(data_dict)
    
    # Now move the index to a column named 'category' and sort by that column.
    df_results = df_results.reset_index().rename(columns={'index': 'category'})
    df_results = df_results.sort_values(by='category')
    return df_results
...
def plot_redux(plot_types,
    ...
    # Retrieve the results from each queue.
    results = {}
    for plot_type, q in result_queues.items():
        res = q.get()        
        data = copy.deepcopy(res)
        if return_imgs:
            # convert any pdfs into images 
            for plot_name,plot_data in res.items():
                for k,value in plot_data.items():
                    if k.endswith("_path") and value.endswith(".pdf"):
                        data[plot_name][k.replace("_path", "_img")] = load_first_page_from_pdf(value)                
        results[plot_type] = data
...
def plot_nonparallel(plot_types,
    ...
    results = {}  # A dict to store results per plot_type
    ...
    for plot_type in plot_types:
        ...
        if plot_type == "confusion-matrix":
            ...
            results['shape_bias_matrixplot'] = plot_shape_bias_matrixplot(datasets=datasets,
                                                                          decision_maker_fun=plotting_definition,
                                                                          result_dir=result_dir)
            
            results['shape_bias_boxplot'] = plot_shape_bias_boxplot(datasets=datasets,
                                    decision_maker_fun=plotting_definition,
                                    result_dir=result_dir)            
        ...
        # If a result_queue was provided, put the result in it
...
def plot_shape_bias_matrixplot(datasets,
    ...
    # Prepare a dictionary to summarize per-decision-maker data
    dmaker_summary = {}
    ...
    for dmaker in decision_maker_fun(df):
        ...
        # Record the computed data for this decision maker.
        dmaker_summary[dmaker.plotting_name] = {"avg": avg, "class_avgs": class_avgs}
    ...
    texture_bias_df = convert_shape_bias_to_texture_bias({
        "sorted_categories": list(classes),
        "decision_maker_summary": dmaker_summary
    })
    
    data_summary = dict(texture_bias_df=texture_bias_df)
    
    return dict(figure_path=figure_path,data_summary=data_summary)
...
def plot_shape_bias_boxplot(datasets,
    ...
    for dmaker in decision_maker_fun(df):
        if len(dmaker.decision_makers) > 1:
            ...
            for dmaker_human in dmaker.decision_makers:
                ...
                current_categories = class_avgs.reset_index().category.tolist()
                if categories == None:
                    categories = current_categories
                else: 
                    assert categories==current_categories, "oops, categories don't match"
    ...
    df_results.insert(0, "category", categories)
    data_summary = {
        "df_results": df_results,
        "decision_maker_to_shape_bias_dict": decision_maker_to_shape_bias_dict,
        "labels": labels,
        "colors": colors,
...
def format_benchmark_df(df, decision_makers, metric_names,
    ...
    if include_humans:
        ...
        df = pd.concat([df, pd.DataFrame([human_dict])], ignore_index=True)
